...
public class DeviceService {
    ...
    public void pairDevice(PairDeviceRequest req) {
        ...
        // GỬI THÔNG BÁO: Gửi riêng cho chính người vừa pair máy
        notificationService.sendAsync(device, user, user, "Ghép thiết bị thành công", "Bạn đã ghép thiết bị " + device.getName());
    }
    ...
    public void shareDevice(ShareDeviceRequest req) {
        ...
        // GỬI THÔNG BÁO CHÍNH XÁC TỚI TỪNG NGƯỜI BẰNG HÀM sendAsync:
        // 1. Gửi cho người được chia sẻ (guestUser)
        notificationService.sendAsync(device, currentUser, guestUser, "Thiết bị mới được chia sẻ", currentUser.getUsername() + " đã chia sẻ " + device.getName() + " cho bạn");
        // 2. Gửi cho chủ sở hữu (currentUser) báo thành công
        notificationService.sendAsync(device, currentUser, currentUser, "Chia sẻ thành công", "Bạn đã chia sẻ " + device.getName() + " cho " + guestUser.getUsername());
    }
    ...
    public void controlDevice(ControlDeviceRequest req) {
        ...
        // TỐI ƯU TẠI ĐÂY: Không dùng vòng lặp for ở đây nữa!
        // Bản thân hàm sendNotification(device, actor, title, content) đã tự tìm danh sách user thuộc thiết bị để bắn FCM rồi.
        notificationService.sendNotification(device, user, "Thiết bị thay đổi", msg);
    }
    ...
    public List<DeviceGuestResponse> getGuests(Long deviceId) {
        ...
        return list.stream().filter(x -> !x.getRole().equals("OWNER"))
                .map(x -> new DeviceGuestResponse(x.getUser().getId(), x.getUser().getUsername(), x.getUser().getEmail(), x.getRole()))
                .toList();
    }
    ...
    public void removeGuest(Long deviceId, Long guestUserId) {
        ...
        // GỬI THÔNG BÁO CHÍNH XÁC CHO TỪNG ĐỐI TƯỢNG BẰNG HÀM sendAsync:
        // 1. Gửi cho Guest (để họ biết mình bị thu hồi quyền)
        notificationService.sendAsync(device, currentUser, guest, "Quyền truy cập bị thu hồi", "Bạn đã bị xóa khỏi " + device.getName());
        // 2. Gửi cho Owner (Chủ máy nhận thông báo báo thành công)
        notificationService.sendAsync(device, currentUser, currentUser, "Đã xóa người dùng", "Đã xóa " + guest.getUsername() + " khỏi " + device.getName());
    }
    ...
    public void renameDevice(RenameDeviceRequest req) {
        ...
        if (!device.getOwner().getId().equals(currentUser.getId())) {
            throw new RuntimeException("Không có quyền đổi tên thiết bị");
        }
